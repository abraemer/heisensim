...
parser.add_argument('--realization', "-r", type=int, default=0,
                    help="Disorder realization.")
...
h_list = [h]
empty_array = np.zeros((1, 1, 1, 2**(N-1)), dtype=np.float64)
empty_array_eev = np.zeros((1, 1, 1, 2**(N-1), N), dtype=np.float64)
empty_array_E_0 = np.zeros((1, 1, 1), dtype=np.float64)
...
simulation_results.to_netcdf(
    cwd.parent / "simulation_results" / "run_N-{}_rbl-{}_h-{}_i-{}.nc".format(
        N, r_bl, h, i)
)
